...
const Paymenttransactions = () => {
    ...
    const fetchPurchasedSubscription = () => {
        setLoading(true);
        axios
            .get(`${url}/proposal/payment-transactions/?search=${onsearchText}&page=${currentPage + 1}&page_size=${rowsPerPage}`, {
                headers: {
    ...
    const fetchPurchasedSubscriptionId = (id) => {
        axios
            .get(`${url}/proposal/payment-transactions/?proposal_id=${id}`, {
                headers: {
                    Authorization: `Bearer ${tokenStr}`,
                    "Content-Type": "application/json",
                },
            ...
            .catch((error) => {
                ...
                axios.post(`${url}/api/token/refresh/`, { refresh: refresh }).then((res) => {
                    ...
                    axios
                        .get(`${url}/proposal/payment-transactions/?proposal_id=${id}`, { headers: new_headers })
                        .then((res) => {
                            ...
                            >
                                <TableHead>
                                    <TableRow>
                                        ...
                                        <TableCell align="center">Commission ($)</TableCell>
                                        <TableCell align="center">Vendor Earnings ($)</TableCell>
                                    ...
                                    {loading ? (
                                        ...
                                        Subscription.map((item, index) => (
                                            ...
                                            >
                                                ...
                                                <TableCell align="center">{item.service_title}</TableCell>
                                                <TableCell align="center">${item.amount_total}</TableCell>
                                                <TableCell align="center">{item.commission_percent}%</TableCell>
                                                <TableCell align="center">${item.commission_amount}</TableCell>
                                                <TableCell align="center">${item.vendor_amount}</TableCell>
                                                <TableCell align="center">
                                                    <Chip
                                                        label={item.status}
                                                        style={{
                                                ...
                                                <TableCell align="center">
                                                    {item.receipt_url ? (
                                                        <a
                                                            ...
                                                            className="text-blue-600 hover:text-blue-800 transition duration-200 inline-flex items-center"
                                                        >
                                                            <OpenInNew
                                                                fontSize="small"
                                                                sx={{
                ...
                >
                    <Grid container spacing={4} mt={1}>
                        ...
                        <Grid item xs={12} md={6}>
                            ...
                            >
                                <Typography variant="subtitle2" color="text.secondary">Service Title</Typography>
                                <Typography>{viewData?.service_title || 'N/A'}</Typography>

                                <Typography variant="subtitle2" color="text.secondary">Description</Typography>
                                <Typography>{viewData?.service_description || 'N/A'}</Typography>

                                <Typography variant="subtitle2" color="text.secondary">Customer</Typography>
                                <Typography>{viewData?.customer || 'N/A'}</Typography>
                                ...
                                <Typography
                                    sx={{
                                        fontWeight: 600,
                                        color: viewData?.status === 'succeeded' ? 'green' : 'red'
                                    }}
                                >
                                    {viewData?.status || 'N/A'}
                                </Typography>
                        ...
                        <Grid item xs={12} md={6}>
                            ...
                            >
                                ...
                                <Typography>${viewData?.amount_total || '0.00'}</Typography>

                                <Typography variant="subtitle2" color="text.secondary">Commission (%)</Typography>
                                <Typography>{viewData?.commission_percent || '0'}%</Typography>

                                <Typography variant="subtitle2" color="text.secondary">Commission Amount</Typography>
                                <Typography>${viewData?.commission_amount || '0.00'}</Typography>

                                <Typography variant="subtitle2" color="text.secondary">Vendor Amount</Typography>
                                <Typography>${viewData?.vendor_amount || '0.00'}</Typography>

                                <Typography variant="subtitle2" color="text.secondary">Created At</Typography>
                                <Typography>
                                    {viewData?.created_at
                                        ? new Date(viewData.created_at).toLocaleString('en-GB')
                                        : 'N/A'}
                                </Typography>
                            </Box>
                        </Grid>
